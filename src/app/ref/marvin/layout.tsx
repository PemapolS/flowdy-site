...
export default function Layout({ children }: { children: React.ReactNode }) {
    ...
    const isActive = (p: string) => pathname === `/ref/marvin/${p}` || pathname.endsWith(p);
    ...
    return (
        <main className="relative bg-white dark:bg-gray-900 min-h-screen flex flex-col">
            <Nav />
            <section className="bg-header-ref-marvin bg-cover bg-top" id="title">
                <div className="px-8 md:px-20 lg:px-60 backdrop-blur-sm">
                    <div className="flex pt-32 pb-6 mx-auto max-w-screen-xl gap-4 flex-col">
                        <h2 className="mb-4 text-4xl tracking-tight font-extrabold text-white font-ibm">Marvin Character Information</h2>
                        <p className="mb-8 sm:text-lg text-gray-300 font-ibm">Reference Sheets and Basic Information about Marvin</p>
                    </div>
                </div>
            </section>
            <div className="py-6 px-8 md:px-20 lg:px-60 flex-1">
                <nav className="flex mx-auto max-w-screen-xl gap-2">
                    <Link href="/ref/marvin/sfw" className={linkClass("/sfw")}>
                        SFW
                    </Link>
                    <Link href="/ref/marvin/nsfw" className={linkClass("/nsfw")}>
                        NSFW
                    </Link>
                </nav>
