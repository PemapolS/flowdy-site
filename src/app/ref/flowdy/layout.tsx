...
export default function Layout({ children }: { children: React.ReactNode }) {
    ...
    const isActive = (p: string) => pathname === `/ref/flowdy/${p}` || pathname.endsWith(p);
        ...
        <main className="relative bg-white dark:bg-gray-900 min-h-screen flex flex-col">
            ...
            <div className="py-6 px-8 md:px-20 lg:px-60 flex-1">
                <nav className="flex mx-auto max-w-screen-xl gap-2">
                    <Link href="/ref/flowdy/sfw" className={linkClass("/sfw")}>
                        SFW
                    </Link>
                    <Link href="/ref/flowdy/nsfw" className={linkClass("/nsfw")}>
                        NSFW
                    </Link>
                </nav>
